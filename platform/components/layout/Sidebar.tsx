...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-5 border-b" style={{ borderColor: 'var(--color-border)' }}>
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] mt-1 font-medium truncate" style={{ color: 'var(--color-text-secondary)' }}>
              {lang === 'ar'
      ...
      {/* مصدر البيانات — نفس التبويب أعلى الصفحة؛ هنا للوضوح */}
      <div className="px-3 py-2.5 border-b" style={{ borderColor: 'var(--color-border)' }}>
        <p className="text-xs mb-2 px-0.5" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        >
          ...
          >
            {lang === 'ar' ? 'سوبرماركت' : 'Supermarket'}
          </button>
          <button
            type="button"
            onClick={() => setDataSource('restaurants')}
            className="flex-1 px-2 py-1.5 text-xs rounded-md transition-colors text-center leading-tight"
            style={{
              background: dataSource === 'restaurants' ? 'var(--color-surface-hover)' : 'transparent',
              color: 'var(--color-text-primary)',
            }}
          >
            {lang === 'ar' ? 'تطبيقات التوصيل' : 'Delivery'}
          </button>
        </div>
      </div>
